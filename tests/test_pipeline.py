...
def test_mission_segmentation_pipeline():
  ...
  # We increase the number of rows for R6 to trigger the B2B deterministic rule
  # (Pezzi > 10 and Valore/Articolo > 150)
  lf = pl.LazyFrame({
  ...
  # Test Elbow Method & Mission Clustering
  # The determine_elbow_method now has internal safety for safe_k
  optimal_k = fe.determine_elbow_method(df, features=mission_features)
  
  # Ensure safe clustering for the test size
  safe_k = min(optimal_k, df.height - 1)
  df = fe.segment_shopping_missions(df, features=mission_features, n_clusters=safe_k)
  ...
  # Italian naming applies here per instructions
  ...
  try:
    ...
    # Verify Italian month mapping (06 -> Giugno)
